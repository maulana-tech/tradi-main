...
async function acceptDirect(handleClient, wallet, publicClient, intentId, buyAmount) {
    const buy = await handleClient.encryptInput(buyAmount, "uint256", ADDRESSES.privateOtc);
    const tx = await wallet.writeContract({
        address: ADDRESSES.privateOtc,
        abi: otcAbi,
        functionName: "acceptIntent",
        args: [intentId, buy.handle, buy.handleProof],
        chain: wallet.chain,
        account: wallet.account,
    });
    await publicClient.waitForTransactionReceipt({ hash: tx });
}
async function cancel(wallet, publicClient, intentId) {
    const tx = await wallet.writeContract({
        address: ADDRESSES.privateOtc,
        abi: otcAbi,
...
async function bid(handleClient, wallet, publicClient, rfqId, amount) {
...
async function finalize(wallet, publicClient, rfqId) {
    const tx = await wallet.writeContract({
        address: ADDRESSES.privateOtc,
        abi: otcAbi,
...
async function main() {
    ...
    ids.openA = await createDirect(bobHandle, bob.wallet, publicClient, ADDRESSES.ceth, ADDRESSES.cusdc, parseUnits("0.6", 18), parseUnits("1750", 6), 24 * 3600);
    console.log(`         → intent #${ids.openA}`);
    console.log("[2x] 2/12 Open Direct — carol cUSDC→cETH 12h");
    ids.openB = await createDirect(carolHandle, carol.wallet, publicClient, ADDRESSES.cusdc, ADDRESSES.ceth, parseUnits("5000", 6), parseUnits("1.4", 18), 12 * 3600);
    console.log(`         → intent #${ids.openB}`);
    // ─────────── Phase 2: 2× Filled Direct ─────────────────────────────
    console.log("[2x] 3/12 Filled Direct — dave cETH→cUSDC, eve accepts");
    ids.filledA = await createDirect(daveHandle, dave.wallet, publicClient, ADDRESSES.ceth, ADDRESSES.cusdc, parseUnits("0.8", 18), parseUnits("2400", 6), 24 * 3600);
    if (ids.filledA !== null) {
    ...
    ids.filledB = await createDirect(frankHandle, frank.wallet, publicClient, ADDRESSES.cusdc, ADDRESSES.ceth, parseUnits("3500", 6), parseUnits("1.0", 18), 24 * 3600);
    if (ids.filledB !== null) {
    ...
    ids.cancelledA = await createDirect(aliceHandle, alice.wallet, publicClient, ADDRESSES.ceth, ADDRESSES.cusdc, parseUnits("1.2", 18), parseUnits("3500", 6), 24 * 3600);
    ...
    ids.cancelledB = await createDirect(bobHandle, bob.wallet, publicClient, ADDRESSES.cusdc, ADDRESSES.ceth, parseUnits("4000", 6), parseUnits("1.1", 18), 24 * 3600);
    ...
    ids.expiredA = await createDirect(carolHandle, carol.wallet, publicClient, ADDRESSES.ceth, ADDRESSES.cusdc, parseUnits("0.3", 18), parseUnits("950", 6), 90);
    console.log(`         → intent #${ids.expiredA} (reads as Expired in ~2min)`);
    console.log("[2x] 8/12 Expired Direct — dave cUSDC→cETH, deadline 60s");
    ids.expiredB = await createDirect(daveHandle, dave.wallet, publicClient, ADDRESSES.cusdc, ADDRESSES.ceth, parseUnits("1500", 6), parseUnits("0.4", 18), 60);
    ...
    ids.rfqOpenA = await createRfq(eveHandle, eve.wallet, publicClient, ADDRESSES.ceth, ADDRESSES.cusdc, parseUnits("0.7", 18), 3600);
    console.log(`         → rfq #${ids.rfqOpenA}`);
    console.log("[2x] 10/12 Open RFQ — frank cUSDC→cETH 2h window");
    ids.rfqOpenB = await createRfq(frankHandle, frank.wallet, publicClient, ADDRESSES.cusdc, ADDRESSES.ceth, parseUnits("6000", 6), 7200);
    ...
    ids.rfqPendingA = await createRfq(aliceHandle, alice.wallet, publicClient, ADDRESSES.ceth, ADDRESSES.cusdc, parseUnits("0.45", 18), 30);
    console.log(`         → rfq #${ids.rfqPendingA}`);
    console.log("[2x] 12/12 PendingReveal RFQ — bob cETH→cUSDC, dave+frank bid");
    ids.rfqPendingB = await createRfq(bobHandle, bob.wallet, publicClient, ADDRESSES.ceth, ADDRESSES.cusdc, parseUnits("0.55", 18), 30);
...
//# sourceMappingURL=seed-2-each.js.map