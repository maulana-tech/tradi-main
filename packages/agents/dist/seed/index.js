...
import { createWalletClient, createPublicClient, http, parseUnits, parseAbi, decodeEventLog, } from "viem";
...
if (!ADDRESSES.privateOtc || !ADDRESSES.cusdc || !ADDRESSES.ceth) {
    throw new Error("Set NEXT_PUBLIC_PRIVATE_OTC_ADDRESS, NEXT_PUBLIC_CUSDC_ADDRESS, NEXT_PUBLIC_CETH_ADDRESS in .env");
}
...
async function main() {
    ...
    await createDirect(handleClient, wallet, publicClient, ADDRESSES.ceth, ADDRESSES.cusdc, parseUnits("1", 18), parseUnits("3000", 6), 24 * 3600);
    console.log("[seed] creating Direct OTC intent: 5000 cUSDC → 1.4 cETH…");
    await createDirect(handleClient, wallet, publicClient, ADDRESSES.cusdc, ADDRESSES.ceth, parseUnits("5000", 6), parseUnits("1.4", 18), 12 * 3600);
    console.log("[seed] (5/5) creating RFQ: 2 cETH for cUSDC, 6h window…");
    await createRfq(handleClient, wallet, publicClient, ADDRESSES.ceth, ADDRESSES.cusdc, parseUnits("2", 18), 6 * 3600);
