...
async function main() {
    ...
    const sellEnc = await bobHandle.encryptInput(parseUnits("1.5", 18), "uint256", ADDRESSES.privateOtc);
    ...
    if (rfqId === null)
        throw new Error("could not parse IntentCreated event");
    console.log(`  ✓ RFQ #${rfqId} tx ${createTx}`);
    /* ──── 7 bids from various wallets ──── */
    const bidPlan = [
        { name: "Deployer", key: adminKey, amount: parseUnits("5400", 6) },
...
//# sourceMappingURL=seed-rfq-7.js.map