...
async function main() {
    ...
    for (let i = start; i < next; i++) {
        ...
        console.log(`#${String(i).padStart(2)} ${MODE_NAMES[mode]?.padEnd(6)} ${STATUS_NAMES[status]?.padEnd(14)} maker=${maker.slice(0, 8)}… deadline ${overdue}${mode === 1 ? `  bids=${bidCount}` : ""}`);
    }
}
main().catch((err) => {
...
//# sourceMappingURL=diagnose-rfq.js.map