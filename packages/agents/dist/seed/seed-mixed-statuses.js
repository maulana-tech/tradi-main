...
async function main() {
    ...
    const openId = await createDirect(aliceHandle, alice.wallet, publicClient, ADDRESSES.ceth, ADDRESSES.cusdc, parseUnits("0.4", 18), parseUnits("1200", 6), 24 * 3600);
    ...
    const filledId = await createDirect(bobHandle, bob.wallet, publicClient, ADDRESSES.cusdc, ADDRESSES.ceth, parseUnits("2000", 6), parseUnits("0.5", 18), 24 * 3600);
    ...
    const cancelledId = await createDirect(daveHandle, dave.wallet, publicClient, ADDRESSES.cusdc, ADDRESSES.ceth, parseUnits("3500", 6), parseUnits("1.0", 18), 24 * 3600);
    ...
    const expiredId = await createDirect(eveHandle, eve.wallet, publicClient, ADDRESSES.ceth, ADDRESSES.cusdc, parseUnits("0.25", 18), parseUnits("750", 6), 90);
    ...
    const rfqOpenId = await createRfq(frankHandle, frank.wallet, publicClient, ADDRESSES.ceth, ADDRESSES.cusdc, parseUnits("0.6", 18), 3600);
    ...
    const rfqPendingId = await createRfq(aliceHandle, alice.wallet, publicClient, ADDRESSES.ceth, ADDRESSES.cusdc, parseUnits("0.5", 18), 30);
...
//# sourceMappingURL=seed-mixed-statuses.js.map