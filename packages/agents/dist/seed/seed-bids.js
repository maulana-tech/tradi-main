...
import { createWalletClient, createPublicClient, http, parseUnits, parseAbi, keccak256, stringToBytes, formatEther, } from "viem";
...
async function main() {
    ...
    const bids = [
        ...
        { rfqId: 11n, bidder: "Deployer", bidderKey: adminKey, amount: parseUnits("3000", 6) },
    ...
    for (const plan of bids) {
        ...
        if (balance < parseUnits("0.001", 18)) {
            console.warn(`[seed-bids] ${plan.bidder} (${acc.address}) low ETH (${formatEther(balance)}), skipping`);
            continue;
        }
        ...
        const enc = await handleClient.encryptInput(plan.amount, "uint256", ADDRESSES.privateOtc);
        try {
...
//# sourceMappingURL=seed-bids.js.map