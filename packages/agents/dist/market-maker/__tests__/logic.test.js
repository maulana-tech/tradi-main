...
import { decideBid, computeBidAmount, } from "../logic.js";
const OUR = "0xMyBot";
const STRATEGY = {
    pairs: {
        "cETH/cUSDC": {
    ...
    maxNotional: 50000n * 10n ** 6n, // 50k cUSDC
    spreadBps: 30,
};
const baseEvent = {
    id: 7n,
...
describe("decideBid", () => {
    describe("positive cases", () => {
        ...
        test("matches pair case-insensitively for sellToken", () => {
            const d = decideBid({ ...baseEvent, sellToken: "0xCETH" }, OUR, STRATEGY);
            expect(d.kind).toBe("bid");
        });
        test("matches pair case-insensitively for buyToken", () => {
            const d = decideBid({ ...baseEvent, buyToken: "0xCUSDC" }, OUR, STRATEGY);
            expect(d.kind).toBe("bid");
        });
    });
    describe("negative cases", () => {
        ...
        test("matches own-address check case-insensitively", () => {
            const d = decideBid({ ...baseEvent, maker: "0xMYBOT" }, "0xmybot", STRATEGY);
            expect(d.kind).toBe("skip");
        });
...
describe("computeBidAmount", () => {
    describe("positive cases", () => {
        test("applies 30 bps spread to 50k notional → 49,850e6", () => {
            const max = 50000n * 10n ** 6n;
            // 50000 * (10000-30)/10000 = 50000 * 9970/10000 = 49850
            expect(computeBidAmount(max, 30)).toBe(49850n * 10n ** 6n);
        });
