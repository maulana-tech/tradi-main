...
async function handleIntent(
  ...
  if (env.WRITER_MODE === "dry-run") {
    console.log(
      `[market-maker] DRY RUN: would bid ${decision.bidAmount} on RFQ #${decision.intentId} (${decision.pairName})`,
    );
    return;
  }
  ...
  console.log(
    `[market-maker] bid ${ok ? "succeeded" : "failed"} on RFQ #${decision.intentId}: tx=${audit.transactionHash} pair=${decision.pairName} (routed via ${audit.routedVia}, sponsored=${audit.sponsored})`,
  );
}
