...
const STATUS = ["Open", "Filled", "Cancelled", "Expired"];
const MODE = ["Direct", "RFQ"];
export const browseIntentsTool = {
    name: "private_otc_browse_intents",
    description: "List currently open OTC intents on PrivateOTC. Returns metadata only; encrypted amounts are NOT decrypted (caller doesn't have ACL).",
    inputSchema: {
        type: "object",
        properties: {
    ...
    async handler(rawArgs) {
        ...
        for (let i = start; i < total; i++) {
            try {
                ...
                // Filters
                if (args.sellToken && r[1].toLowerCase() !== args.sellToken.toLowerCase())
                    continue;
                if (args.buyToken && r[2].toLowerCase() !== args.buyToken.toLowerCase())
                    continue;
                if (args.mode === "direct" && r[7] !== 0)
                    continue;
                if (args.mode === "rfq" && r[7] !== 1)
                    continue;
                if (r[6] !== 0)
                    continue; // only Open
                rows.push({
...
//# sourceMappingURL=browseIntents.js.map