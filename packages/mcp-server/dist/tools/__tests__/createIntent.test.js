...
const OTC = "0x000000000000000000000000000000000000000A";
...
function setupHappyPath(opts = {}) {
    ...
    if (intentIdInLog !== null) {
        ...
        mockDecodeEventLog.mockImplementation(() => {
            ...
            if (idx === 0) {
                return { eventName: "Other", args: { id: 99n } };
            }
            return {
...
describe("createIntentTool.handler", () => {
    describe("positive cases", () => {
        ...
        test("encrypts both sellAmount and minBuyAmount as BigInts against PrivateOTC address", async () => {
            ...
            expect(encryptInput).toHaveBeenNthCalledWith(1, 1000000000000000000n, "uint256", OTC);
            expect(encryptInput).toHaveBeenNthCalledWith(2, 3500000000n, "uint256", OTC);
        });
    ...
    describe("negative cases", () => {
        test("rejects when sellToken is not a 0x address", async () => {
            await expect(createIntentTool.handler({ ...baseInput, sellToken: "not-an-address" })).rejects.toThrow();
        });
        test("rejects when sellToken is too short", async () => {
            await expect(createIntentTool.handler({ ...baseInput, sellToken: "0x1234" })).rejects.toThrow();
        });
        test("rejects when buyToken is not a 0x address", async () => {
            await expect(createIntentTool.handler({ ...baseInput, buyToken: "0xZZZ" })).rejects.toThrow();
        });
        ...
        test("rejects when deadlineSeconds is zero (positive() in zod requires > 0)", async () => {
            await expect(createIntentTool.handler({ ...baseInput, deadlineSeconds: 0 })).rejects.toThrow();
        });
        test("rejects when deadlineSeconds is negative", async () => {
            await expect(createIntentTool.handler({ ...baseInput, deadlineSeconds: -100 })).rejects.toThrow();
        });
        test("rejects when sellAmount is not a string", async () => {
            await expect(createIntentTool.handler({ ...baseInput, sellAmount: 1000 })).rejects.toThrow();
        });
        ...
        test("swallows decoder throw and continues to find IntentCreated", async () => {
            ...
            mockDecodeEventLog.mockImplementation(() => {
                ...
                return { eventName: "IntentCreated", args: { id: 42n } };
            });
            const result = await createIntentTool.handler(baseInput);
    ...
    describe("edge cases", () => {
        ...
        test("handles very large bigint string at uint256 max", async () => {
            ...
            expect(encryptInput).toHaveBeenNthCalledWith(1, 2n ** 256n - 1n, "uint256", OTC);
        });
...
//# sourceMappingURL=createIntent.test.js.map