...
function setupClient(opts) {
    const readContract = vi.fn().mockImplementation(async (params) => {
        if (params.functionName === "nextIntentId")
            return opts.nextIntentId;
        if (params.functionName === "intents") {
            const i = Number(params.args[0]);
...
describe("browseIntentsTool.handler", () => {
    describe("positive cases", () => {
        ...
        test("filters by sellToken (case-insensitive)", async () => {
            ...
            const rows = parseRows(await browseIntentsTool.handler({ sellToken: "0xaaaa" }));
        ...
        test("filters by buyToken (case-insensitive)", async () => {
            ...
            const rows = parseRows(await browseIntentsTool.handler({ buyToken: "0xcccc" }));
            expect(rows).toHaveLength(1);
        ...
        test("filters by mode=direct (mode 0 only)", async () => {
            ...
            const rows = parseRows(await browseIntentsTool.handler({ mode: "direct" }));
            expect(rows).toHaveLength(1);
        ...
        test("filters by mode=rfq (mode 1 only)", async () => {
            ...
            const rows = parseRows(await browseIntentsTool.handler({ mode: "rfq" }));
            expect(rows).toHaveLength(1);
        ...
        test("mode=all returns both Direct and RFQ open intents", async () => {
            ...
            const rows = parseRows(await browseIntentsTool.handler({ mode: "all" }));
            expect(rows).toHaveLength(2);
        });
        test("respects custom limit parameter", async () => {
            ...
            const intentReads = readContract.mock.calls.filter((c) => c[0].functionName === "intents");
    ...
    describe("negative cases", () => {
        test("rejects when limit is below 1 via zod", async () => {
            ...
            await expect(browseIntentsTool.handler({ limit: 0 })).rejects.toThrow();
        });
        test("rejects when limit exceeds 50 via zod", async () => {
            setupClient({ nextIntentId: 0n });
            await expect(browseIntentsTool.handler({ limit: 51 })).rejects.toThrow();
        });
        test("rejects when mode is not one of direct/rfq/all", async () => {
            setupClient({ nextIntentId: 0n });
            await expect(browseIntentsTool.handler({ mode: "invalid" })).rejects.toThrow();
        });
...
//# sourceMappingURL=browseIntents.test.js.map