...
import { getPublicClient, getHandleClient, getWalletClient, } from "../../client.js";
...
function setupHappyPath(opts = {}) {
    ...
    mockGetHandleClient.mockResolvedValue({ decrypt });
...
describe("decryptBalanceTool.handler", () => {
    ...
    describe("negative cases", () => {
        test("rejects when tokenAddress is missing", async () => {
            await expect(decryptBalanceTool.handler({})).rejects.toThrow();
        });
        test("rejects non-0x-prefixed addresses", async () => {
            await expect(decryptBalanceTool.handler({ tokenAddress: "deadbeef" })).rejects.toThrow();
        });
        test("rejects 0x with too few hex chars", async () => {
            await expect(decryptBalanceTool.handler({ tokenAddress: "0xabc" })).rejects.toThrow();
        });
        test("rejects 0x with non-hex chars", async () => {
            await expect(decryptBalanceTool.handler({
        ...
        test("propagates readContract failure (RPC down)", async () => {
            ...
            await expect(decryptBalanceTool.handler({ tokenAddress: TOKEN })).rejects.toThrow("rpc unreachable");
        });
        test("propagates decrypt failure (e.g. caller not on ACL)", async () => {
            setupHappyPath({
                decryptThrows: new Error("not authorized"),
            });
            await expect(decryptBalanceTool.handler({ tokenAddress: TOKEN })).rejects.toThrow("not authorized");
        });
    });
    describe("edge cases", () => {
...
//# sourceMappingURL=decryptBalance.test.js.map